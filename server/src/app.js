...
app.delete('/bookings/:bookingId', async (req, res) => {
  try {
    ...
    // Find the booking with the provided ID and delete it
    const deletedBooking = await models.Booking.findByIdAndDelete(bookingId);

    if (!deletedBooking) {
      // If the booking was not found, send a 404 response
      return res.status(404).json({ error: 'Booking not found.' });
    }

    // Respond with a success message or the deleted booking if needed
    res.json({ message: 'Booking successfully deleted.', deletedBooking });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
